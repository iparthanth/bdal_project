...
A `pom.xml` is provided per module for anyone who prefers `mvn package`.
...
```
TotalSalesByProduct/     pom.xml, src/main/java/totalsalesbyproduct/
AverageInvoiceValue/     pom.xml, src/main/java/averageinvoicevalue/
TopCustomersBySpending/  pom.xml, src/main/java/topcustomersbyspending/
BinningByCountry/        pom.xml, src/main/java/binningbycountry/
PigAnalysis/scripts/     five .pig scripts
```

The dataset, compiled classes, jars and screenshots are not committed; they are
